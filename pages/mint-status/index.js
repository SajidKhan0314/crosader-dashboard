...
export async function getServerSideProps(context) {
  const response = await customAxios.get("/mint-status");
  return {
...
const Index = ({ data }) => {
  ...
  const createMintStatus = () => {
    router.push("/mint-status/create");
  };
      ...
      <Layout>
        <section className="h-full mb-4">
          <div className="flex justify-between items-center w-full">
            ...
            <Button
              text={"Create Mint Status"}
              onClickHandler={createMintStatus}
            />
          </div>
          {/* Table */}
                      ...
                      >
                        <div className="flex gap-4 mt-1 w-fit">
                          <button
                            type="button"
                            onClick={() => {
                              router.push(`/mint-status/${item._id}/edit`);
                            }}
